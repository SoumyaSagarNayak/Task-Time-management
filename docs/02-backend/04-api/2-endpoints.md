...
Retrieve a CSV or PDF file containing raw time entries matching the same filters as the list endpoint. This is used by the frontend reports page for download.

```http
GET /time-entries/export?startDate=2025-01-01&endDate=2025-01-31&format=csv
```
...
- `startDate` (optional, ISO 8601 string) - Filter entries after this date
- `endDate` (optional, ISO 8601 string) - Filter entries before this date
- `format` (required, string) â€“ `csv` or `pdf` indicating output format

**Responses:**
- `200 OK` â€“ Returns binary file; Content-Type will be `text/csv` or `application/pdf` depending on format
