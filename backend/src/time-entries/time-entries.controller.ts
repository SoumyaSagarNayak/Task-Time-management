...
export class TimeEntriesController {
  ...
  @Get('export/pdf')
  @ApiOperation({ summary: 'Export time entries as PDF' })
  @ApiResponse({ status: 200, description: 'PDF file' })
  async exportPDF(@Query() query: QueryTimeEntriesDto, @CurrentUser() user: CurrentUserData, @Res() res: Response) {
    const pdf = await this.timeEntriesService.exportPDF(query, user);
  ...
  @Get('export/csv')
  @ApiOperation({ summary: 'Export time entries as CSV' })
  @ApiResponse({ status: 200, description: 'CSV file' })
  async exportCSV(@Query() query: QueryTimeEntriesDto, @CurrentUser() user: CurrentUserData, @Res() res: Response) {
    const csv = await this.timeEntriesService.exportCSV(query, user);
